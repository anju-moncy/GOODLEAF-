...
MODEL_POTATO = tf.keras.models.load_model("Goodleaf/Models/Potato.h5")
...
POTATO_DISC = [
    "Affects leaves, stems and tubers and can reduce yield, tuber size, storability of tubers, quality of fresh-market and processing tubers and marketability of the crop",
    "Infect potato foliage and tubers at any stage of crop development",
    "None",
]
POTATO_TREAT = [
    "Thoroughly spray the plant (bottoms of leaves also) with Bonide Liquid Copper Fungicide concentrate",
    "Fungicides that contain maneb, mancozeb, chlorothanolil, or fixed copper",
    "None",
]
POTATO_PREVENTION = [
    "Planting potato varieties, Avoid overhead irrigation and allow for sufficient aeration between plants to allow the foliage to dry as quickly as possible",
    "Eliminating cull piles and volunteer potatoes, using proper harvesting and storage practices, and applying fungicides when necessary, Air drainage to facilitate the drying of foliage each day is important",
    "None",
]
...
MODEL_APPLE = tf.keras.models.load_model("Goodleaf\Models\Apple.h5")
APPLE_CAUSE = [
...
APPLE_DISC = [
    "Overwinters on fallen diseased leaves. In spring, these fungi shoot spores into the air. Spores are carried by wind to newly developing leaves, flowers, fruit or green twigs",
...
APPLE_TREAT = [
    "Fungicide application must begin in early spring from apple green tip, and continue on a 7- to 10-day schedule (7 days during wet weather, 10 days if dry) until petal fall. If dry weather persists after petal fall, a 10- to 14-day spray schedule is adequate for scab control.",
...
APPLE_PREVENTION = [
    "Choose scab-resistant varieties of apple or crabapple trees, Rake up and discard any fallen leaves or fruit on a regular basis, and never leave fallen leaves or fruit on the ground over winter",
...
async def predict(file: UploadFile = File(...)):
    ...
    # y = json.dumps(x, casue)
    # print(y)
    return x


MODEL_CHERRY = tf.keras.models.load_model("Goodleaf\Models\Cherry.h5")
...
MODEL_CORN = tf.keras.models.load_model("Goodleaf\Models\Corn.h5")
...
CORN_TREAT = [
    ...
    "By spraying with a mild solution of bicarbonate of soda (baking soda), using ½ teaspoon per gallon (2.5 mL. per 4 L.) of water",
    "None",
]
CORN_PREVENTION = [
    "Reduce thatch layer. Irrigate deeply, but infrequently. This generally means one time per week with one inch of water. Always irrigate in the morning, which promotes quick drying of the foliage. Avoid using post-emergent weed killers on the lawn while the disease is active.",
...
MODEL_GRAPE = tf.keras.models.load_model("Goodleaf\Models\Grape.h5")
GRAPE_CLASS_NAMES = [
    "Black Rot",
    "Esca",
    "Leaf Blight",
...
GRAPE_DISC = [
    "It is one of the most common diseases of grapes in areas where the growing season is warm and humid",
    "Symptoms first become apparent in vineyards 5 to 7 or more years old, but the infections actually occur in younger vines",
    "Bacterial blight of grapevine is a serious, chronic and systemic disease of grapevine that affects commercially important cultivars",
    "None",
]
GRAPE_TREAT = [
    "Mancozeb, and Ziram are all highly effective against black rot",
    "Captan and Myclobutanil are the fungicides of choice.",
    "Lime sulfur, Sulfur or Copper formulations",
    "None",
]
GRAPE_PREVENTION = [
    "Before buying crop seeds call your seed company and make sure they can certify that their seed is disease free",
    "Use alternative pruning methods such as delayed pruning or double pruning. Avoid pruning during periods of heavy rainfall when spores are likely to be dispersed. Monitor the vineyard in Spring, and look for dead spurs or for stunted shoots.",
...
"""
MODEL_PEACH = tf.keras.models.load_model("Goodleaf\Models\Peach.h5")
...
PEACH_PREVENTION = [
    "Transplants should be inspected regularly to identify symptomatic seedlings. Transplants with symptoms may be removed and destroyed or treated with streptomycin",
    "None",
]
...
MODEL_PEPPER = tf.keras.models.load_model("Goodleaf\Models\Pepper.h5")
PEPPER_CLASS_NAMES = ["Bacterial Spot", "Healthy"]
PEPPER_CAUSE = ["Most Xanthomonas Baterial Family", "None"]
PEPPER_DISC = [
    "Bacterial spot affects peaches, nectarines, apricots, plums, peppers and cherries. The disease is widespread throughout all fruit growing peppers",
    "None",
]
PEPPER_TREAT = [
    "Copper, Oxytetracycline (Mycoshield and generic equivalents), and Syllit+Captan;",
...
MODEL_STRAWBERRY = tf.keras.models.load_model("Goodleaf\Models\Strawberry.h5")
STRAWBERRY_CLASS_NAMES = ["Leaf Scorch", "Health"]
STRAWBERRY_CAUSE = ["Fungus [Diplocarpon Earlianum]", "None"]
STRAWBERRY_DISC = [
    "This ascomycete produces disk-shaped, dark brown to black apothecia (0.25-1 mm) on advanced-stage lesions on strawberry leaves and leaf residues (Heidenreich and Turechek).",
    "None",
]
STRAWBERRY_TREAT = ["No Known Cure", "None"]
STRAWBERRY_PREVENTION = [
    "Prevention of scorch needs to begin with winter watering. A deep soaking once a month, when there is no snow cover, will help prevent root die-back due to dehydration.",
    "None",
]
